...
unsigned long long poszukiwacz(int height, int width){
...
if(height == 1 || width == 1){
 ...
 memo[height][width] = poszukiwacz(height-1, width) + poszukiwacz(height, width-1);
return memo[height][width];
}
...
iloscDrog = poszukiwacz(height, width);
